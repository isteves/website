...
* the cumulative amount spent at each point in time (we id's represent the sequence of events)
...
However, the table above is actually missing R's equivalents of `OVER()` and `ORDER BY`. Because they did not need to be specified for each variable separately, I was able to extract out all the `ORDER BY o_id`s to a single `arrange(o_id)` before the `mutate()`. For the ranking functions, I expressed the `ORDER BY spent` directly in the arguments. If I had needed to, I could have specified the ordering explicitly with `order_by()`. 

In my book, the ability to extract out common actions makes the tidyverse solution superior for this example. We'll see if SQL can win me over once I need customized windows for each variable...
